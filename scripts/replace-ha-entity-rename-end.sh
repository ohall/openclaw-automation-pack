#!/bin/bash
# Replace the end of ha-entity-rename.mjs with standardized exit codes

# First, add the import at the top (after the other imports)
sed -i "/import { getValidatedEnv } from '.\/_env.mjs'/a import { ExitCodes, exitWithError, exitWithSuccess, exitWithDryRun } from '.\/_exit-codes.mjs';" scripts/ha-entity-rename.mjs

# Now replace from "if (jsonOutput) {" at line 324 to the end
# We'll do this by creating a new file
head -323 scripts/ha-entity-rename.mjs > scripts/ha-entity-rename-new.mjs

# Add the new ending
cat << 'EOF' >> scripts/ha-entity-rename-new.mjs
if (jsonOutput) {
      const result = {
...
mv scripts/ha-entity-rename-new.mjs scripts/ha-entity-rename.mjs

echo "Updated ha-entity-rename.mjs with standardized exit codes"