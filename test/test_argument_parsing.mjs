...
test('ha-entity-rename.mjs accepts --dry-run flag', async () => {
  ...
  try {
    const result = await runScript(
      ...
      ['--from', 'sensor.old', '--to', 'sensor.new', '--dry-run'],
      {
        HA_ENV_FILE: envFile,
      }
    );
...
test('ha-entity-rename.mjs accepts --json flag', async () => {
  ...
  try {
    const result = await runScript(
      'ha-entity-rename.mjs',
      ['--from', 'sensor.old', '--to', 'sensor.new', '--json', '--dry-run'],
      {
        HA_ENV_FILE: envFile,
      }
    );
...
test('ha-backup-config.mjs accepts --output-dir with path', async () => {
  ...
  try {
    const result = await runScript(
      'ha-backup-config.mjs',
      ['--output-dir', '/tmp/test-backup', '--dry-run'],
      {
        HA_ENV_FILE: envFile,
      }
    );
