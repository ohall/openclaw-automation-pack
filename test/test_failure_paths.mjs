...
test('ha-entity-rename.mjs fails with invalid entity ID format', async () => {
  ...
  try {
    const result = await runScript('ha-entity-rename.mjs', [
      '--from', 'invalid_entity',  // Missing domain
      '--to', 'sensor.new',
      '--dry-run'
    ], {
      HA_ENV_FILE: envFile
...
test('ha-entity-rename.mjs fails when --backup-dir is not writable', async () => {
  ...
  try {
    ...
    const result = await runScript('ha-entity-rename.mjs', [
      '--from', 'sensor.old',
      '--to', 'sensor.new',
      '--backup-dir', '/root/backups',  // Should fail
      '--dry-run'
    ], {
      HA_ENV_FILE: envFile
    });
...
test('ha-disable-orphans.mjs fails with invalid allowlist file', async () => {
  ...
  try {
    ...
    // Should fail with non-zero exit code
    assert.notStrictEqual(result.code, 0);
    assert(result.stderr.includes('Allowlist must be a JSON array') || result.stderr.includes('Failed to load allowlist'));
  } finally {
    unlinkSync(envFile);
...
test('ha-disable-orphans.mjs fails with non-existent allowlist file', async () => {
  ...
  try {
    ...
    // Should fail with non-zero exit code
    assert.notStrictEqual(result.code, 0);
    assert(result.stderr.includes('Failed to load allowlist') || result.stderr.includes('no such file'));
  } finally {
    unlinkSync(envFile);
  }
});
...
test('ha-restart-and-wait.mjs fails with invalid timeout value', async () => {
  ...
  try {
    const result = await runScript('ha-restart-and-wait.mjs', [
...
test('ha-hacs-update.mjs fails with invalid --auto-restart value', async () => {
  ...
  try {
    const result = await runScript('ha-hacs-update.mjs', [
      '--auto-restart', 'invalid'  // Should be boolean
    ], {
      HA_ENV_FILE: envFile
    });
...
test('scripts exit with appropriate error codes for different failures', async () => {
  ...
  try {
    ...
    const result1 = await runScript('ha-entity-rename.mjs', [
      '--unknown-flag'
    ], {
      HA_ENV_FILE: envFile
    });
